...
import cn.lastwhisper.modular.service.activityService.ActUserService;
...
public class UserServiceImpl implements UserService {
	...
	public GlobalResult addUser(User user) {
		...
		user.setUser_id(IDUtil.getUUID());
		Integer integer = userMapper.insertUser(user);
		actUserService.insertUser(user);
		if (integer == 0) {
	...
	@RequiresPermissions("用户管理")
	@Override
	public GlobalResult deleteUser(String user_id) {
	...
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	@Override
	public List<EasyUIOptionalTreeNode> findUserRole(String user_id) {
		// 1.获取当前用户的所有角色
	...
	@Transactional(propagation = Propagation.NOT_SUPPORTED)
	@Override
	public List<Role> findUserRoleByUserid(String user_id) {
		return userMapper.selectUserRole(user_id);
	}

	@LogAnno(operateType = "更新用户对应角色")
	@Override
	public GlobalResult updateUserRole(String user_id, String checkedIds) {
		try (Jedis jedis = jedisPool.getResource()) {
			// 先删除用户下的所有角色
			userMapper.deleteUserRole(user_id);
			if (checkedIds != null) {
				...
				for (String roleuuid : ids) {
					// 设置用户的角色
					userMapper.insertUserRole(user_id, roleuuid);
					identityService.createMembership(user_id,roleuuid);
				}
			}
			// 清除缓存
			jedis.del("menusEasyui_" + user_id);
